...
### Virtual Storage
- Instead of storing files in local storage devices in your computer, files can be stored **remotely**.
- This means that multiple storage devices act like one in order to store your data. These storage devices are collectively known as **virtual storage**.
- Virtual storage allows users to store large quantities of data without needing the harware in their homes.
- However, it can come woth high costs, as well as depending greatly on the user's network speed.
- One example of virtual storage is Google Drive.
